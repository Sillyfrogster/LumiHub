...
import { buildPresetExport, type NormalizedPresetImport } from '../schemas/preset.schema.ts';

const repo = () => AppDataSource.getRepository(Preset);

export interface PresetCreateData extends NormalizedPresetImport {
  name?: string;
  description?: string;
  tags?: string[];
...
export async function listPresets(params: ListQueryParams) {
  ...
  if (params.search) {
    qb.andWhere('(preset.name ILIKE :search OR preset.description ILIKE :search)', { search: `%${params.search}%` });
  }
...
export async function createPreset(data: PresetCreateData) {
  const preset = repo().create({
    name: data.name || String(data.preset.name || 'Untitled Preset'),
    description: data.description ?? String(data.preset.description || ''),
    preset: data.preset,
    tags: data.tags ?? [],
...
export async function updatePreset(id: string, data: Partial<PresetCreateData>) {
  ...
  repo().merge(existing, {
    ...(data.name !== undefined ? { name: data.name || String(data.preset?.name || existing.name) } : {}),
...
export function buildExport(preset: Preset) {
  return buildPresetExport({
...
async function deleteImageFile(relativePath: string) {
  ...
  try {
    ...
    logger.warn(`Could not delete preset image: ${relativePath}`, err);
  }
}
