...
func TestAnUnknownKindHasNoContentFloor(t *testing.T) {
	if checks := ContentFloor("unknown", nil); len(checks) != 0 {
		t.Errorf("unknown floor = %+v, want no requirements", checks)
	}
}
