...
/**
 * A person's writing, as the page shows it. Markdown is read down to emphasis,
 * headings, lists, links, block quotes and inline code, and markup a creator
 * wrote is reduced to the words inside it.
 */
...
/** Said once beside the content it happened to, in plain words. */
...
function Block({ block }: { block: RichBlock }) {
  ...
  if (block.kind === "heading") {
    // A block title is already an h2 and an element label an h3, so a creator's
    // own headings start below both and stop where HTML does.
